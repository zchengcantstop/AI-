...
Do * Find(Othello *board, enum Option player, int step, int min, int max, Do *choice)
{
	...
	num = board->Rule(board, player); //判断落子是否符合规则 并得到可以落子的地点数 
	if (num == 0)    /* 无处落子 */
	{
		if (board->Rule(board, (enum Option) - player))    /* 对方可以落子,让对方下.*/
		{
			...
			pNextChoice = Find(&tempBoard, (enum Option) - player, step - 1, -max, -min, pNextChoice);  //进入对方的搜索函数 
	...
	{
		choice->score = board->Judge(board, player);  //得到评估函数的得分 
		return choice;
	}
    //轮到自己选择落子 
	allChoices = (Do *)malloc(sizeof(Do)*num);  //所有的选择 
	k = 0;
	for (i = 0; i<6; i++)
	{
		for (j = 0; j<6; j++)  //从角落选择 
	...
	for (k = 0; k<num; k++)
	{
		...
		pNextChoice = Find(&tempBoard, (enum Option) - player, step - 1, -max, -min, pNextChoice); //递归搜索 
		...
		else if (thisChoice.score >= max)    //如果>MAX,剪枝 
		{
			choice->score = thisChoice.score;
			choice->pos.first = thisChoice.pos.first;
			choice->pos.second = thisChoice.pos.second;
...
int Othello::Judge(Othello *board, enum Option player)  //评估函数（修改后） 
{   
    WinNum grid[6][6];   
    for(int i=0;i<6;i++){
    	for(int j=0;j<6;j++){
	...
	int X1[] = {-1, -1, 0, 1, 0, -1};  
	int Y1[] = {0, 1, 1, -1, -1, -1};  
	int V[6][6]={{20, -3, 11, 11, -3, 20}  //构建新的系数数列 
    	,{-3, -7, -4, -4, -7, -3}
    	,{11, -4, 2, 2, -4, 11}
...
// Piece difference 件差, frontier disks边界 and disk squares  磁盘正方形 
	for(i=0; i<6; i++)
		for(j=0; j<6; j++)  {
			...
			if(grid[i][j].color != 0)   {
				for(k=0; k<6; k++)  {
					x = i + X1[k]; y = j + Y1[k];
					if(x >= 0 && x < 6 && y >= 0 && y < 6 && grid[x][y].color == SPACE) {
						if(grid[i][j].color == my_color)  my_front_tiles++;
	...
	if(my_tiles > opp_tiles)  //件差 
		p = (100.0 * my_tiles)/(my_tiles + opp_tiles);
	else if(my_tiles < opp_tiles)
		p = -(100.0 * opp_tiles)/(my_tiles + opp_tiles);
	else p = 0;

	if(my_front_tiles > opp_front_tiles)  //边界 
